...
int charIndex(const char* s) noexcept {
	return ((s[0] - '@') * 676 + (s[1] - '@') * 26 + (s[2] - '@'));
}

// Efficiently returns index from char[3] s
int intIndex(const char* s) noexcept {
...
int main(int argc, char* argv[]) {
	...
	#pragma omp parallel
	{
		// Set true at [c][n] where c is base26-index and n normal base10 if regnr exists
		#pragma omp for private (n, c)
		for (int i = 0; i < lSize; i += 8) {
			c = charIndex(&buffer[i]);
			n = intIndex(&buffer[i] + 3);
			matrix[c][n] = true;
		}
		// Go through list and count if number of unique id's matches number of entries
		#pragma opm for reduction (+:abort)
